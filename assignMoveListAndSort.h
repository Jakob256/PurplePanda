...
void assignMoveListAndSort (int board[8][8],unsigned long long int key, int moveList[], bool sortMoves){	// we use the 5 integer notation:
	...
	*******************************************/
	...
	for (int i=1; i<=moveList[0]; i++){
		...
		if (startpiece>=5){startpiece=10;}
		if (startpiece==2){startpiece=5;}
		if (startpiece==4){startpiece=3;}


		endpiece=abs(endpiece);
		if (endpiece>=5){endpiece=10;} // however, endpiece==King should not be possible
		if (endpiece==2){endpiece=5;}
		if (endpiece==4){endpiece=3;}

		// now comes the difficult and critical part:
		
		///// usual way: /////
		quickEvals[i]=endpiece-startpiece*(isAttacked[moveList[5*i-2]][moveList[5*i-1]])+10;  // takes values from 0-10+10=0 to 10-0+10=20
		
		///// all captures first: /////
		//quickEvals[i]=(moveList[5*i]!=0)*11+endpiece-startpiece*(isAttacked[moveList[5*i-2]][moveList[5*i-1]])+10;  // takes values from 0-10+10=0 to 11+10-0+10=31

		
		if ((quickEvals[i]<0) || (quickEvals[i]>31)){
